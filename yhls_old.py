...
dizhi1 = r'/Users/win11/Desktop/2.3银行流水/银行流水分析'

result = pd.ExcelWriter(r'/Users/win11/Desktop/2.3银行流水/1.xlsx')
result1 = pd.ExcelWriter(r'/Users/win11/Desktop/2.3银行流水/2.xlsx')
result2 = pd.ExcelWriter(r'/Users/win11/Desktop/2.3银行流水/3.xlsx')
result3 = pd.ExcelWriter(r'/Users/win11/Desktop/2.3银行流水/6.xlsx')
result4 = pd.ExcelWriter(r'8.xlsx', engine='xlsxwriter')
result9 = pd.ExcelWriter(r'/Users/win11/Desktop/2.3银行流水/888.xlsx')
...
def rd_data(a, ls):
    ...
    for i, value in enumerate(origin_file_list):
        # 拼接每个文件的路径
        file_path = '/Users/win11/Desktop/2.3银行流水/银行流水分析/%s/%s' % (a, value)
...
for i in excel_list:
    ...
    for j in i:
        print(j)
        file_path = '/Users/win11/Desktop/2.3银行流水/银行流水分析/%s/%s' % (a, j)
        m = skip_r(file_path)
        n = skip_f(file_path, m)
    ...
    ls_yhls[a] = ls0
    ls_yhls1[a] = ls0
    ls1 = pd.concat([ls1, ls0], axis=0, join='outer')
    a+=1
...
print(company_name)
if company_name ==0:
   company_name = '蒙羊牧业股份有限公司'

glqy = pd.read_excel(r'/Users/win11/Desktop/2.3银行流水/关联企业-%s.xlsx'%(company_name), skiprows=0)
...
ls_p2p_list = pd.read_excel(r'/Users/win11/Desktop/2.3银行流水/P2P名单.xlsx', skiprows=0)
...
if gongzi > 0:
    ...
    print('hhhhhhhhhhh')
    ls_gz = pd.read_excel(r'/Users/win11/Desktop/2.3银行流水/全国各城市工资水平.xlsx', skiprows=0)
...
f_out = open(r'/Users/win11/PycharmProjects/YHLS/num.txt', 'r+')
bianma = f_out.read()
bianma = int(bianma)
